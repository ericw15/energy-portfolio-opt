...
XLE_TICKERS = [
    "XOM", "CVX", "COP", "PSX", "MPC", "VLO", "SLB", "EOG", "WMB", "BKR",
    "KMI", "OXY", "HAL", "FANG", "DVN", "TRGP", "OKE", "APA",
    "NOV"
]
...
def main():
    ...
    equal_weighting = np.full((baseline_portfolio_returns.size, len(XLE_TICKERS)),
                                fill_value=1/len(XLE_TICKERS))
    xle_individual_asset_returns = get_returns(tickers=XLE_TICKERS, 
                start_date=start, 
                end_date=end)
    equal_weighting_returns_indiv = np.sum(xle_individual_asset_returns * equal_weighting, axis=1) + 1
    # TODO is this +1 in the right place?
    equal_weighting_returns = np.cumprod(equal_weighting_returns_indiv)

    pca_portfolio_balancing = pd.DataFrame(np.full(equal_weighting.shape, np.nan), index=equal_weighting_returns.index)
    ...
    while current < end:
        ...
        to_be_replaced = (pca_portfolio_balancing.index >= current) & (pca_portfolio_balancing.index < current + step) # only get trading days in that period

        current_equity_data = get_returns(tickers=XLE_TICKERS,start_date=start_training_data, end_date=current)
        longer_equity_data = get_returns(tickers=XLE_TICKERS,start_date=start_training_data, end_date=current)
        pca_factor_strat = PCA_factor_Strategy(risk_free_rate= 0.04 / 252)
        expected_returns_lgbm = get_lightgbm_ER(equity_data=longer_equity_data, 
                                                window_size=lgbm_lookback)
        cov_matrix = pca_factor_strat.get_covariance_matrix(start_date=start, 
                                                            end_date=current, 
                                                            equity_data=current_equity_data)
        #sns.heatmap(cov_matrix, cmap="coolwarm", xticklabels=current_equity_data.columns, yticklabels=equity_data.columns)
        #plt.savefig("PCA_factor_covariance.png")
        portfolio = pca_factor_strat.optimize_towards_sharpe_ratio(cov_matrix, expected_returns_lgbm, equity_names=current_equity_data.columns)
        pca_portfolio_balancing[to_be_replaced] = np.tile(portfolio.x, (np.sum(to_be_replaced), 1))
    ...
    period_performance = np.sum(xle_individual_asset_returns * pca_portfolio_balancing,axis=1)
    ...
    portfolios_return_by_day["Equal Weighting, Equity"] = equal_weighting_returns.to_numpy().flatten()
    portfolios_return_by_day["Baseline (XLE)"] = baseline_portfolio_returns.to_numpy().flatten()
    portfolios_return_by_day["PCA Factor with LightGBM"] = overall_pca_factor_portfolio_returns.to_numpy().flatten()
    plt.figure(figsize=(12,6))
    sns.lineplot(data=pd.DataFrame(portfolios_return_by_day, index=baseline_portfolio_returns.index))
