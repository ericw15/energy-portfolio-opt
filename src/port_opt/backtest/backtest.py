...
def main():
    ...
    lgbm_lookback = 30
    current = start + timedelta(weeks=52)
    ...
    while current < end:
        ...
        current_equity_data = get_returns(tickers=XLE_TICKERS,start_date=start, end_date=current)
        ...
        expected_returns_lgbm = get_lightgbm_ER(equity_data=longer_equity_data, window_size=lgbm_lookback)
        cov_matrix = pca_factor_strat.get_covariance_matrix(start_date=start, end_date=current, equity_data=current_equity_data)
        ...
        # period_performance = np.sum(current_equity_data * portfolio.x,axis=1)
        breakpoint()
        previous_portfolio_performance = pd.concat([previous_portfolio_performance, period_performance])
    ...
    overall_performance = previous_portfolio_performance + 1
    ...
    equal_weighting_returns_indiv = np.sum((get_returns(tickers=XLE_TICKERS, 
                                                         start_date=start, 
                                                         end_date=end)) * equal_weighting, axis=1) + 1
